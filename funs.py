...
def get_feature_matrix(sales, test, items, list_lags, date_block_threshold):
    ...
    for block_num in sales['date_block_num'].unique():
        cur_shops = sales.loc[sales['date_block_num'] == block_num, 'shop_id'].unique() # getting unique shop ids for each date_block_num
        cur_items = sales.loc[sales['date_block_num'] == block_num, 'item_id'].append(pd.Series(cur_items_aux)).unique() # getting unique item ids which are also in the test set
        cur_items_aux = cur_items[pd.Series(cur_items).isin(test.item_id)] # taking all the item ids which are also in the test set.
